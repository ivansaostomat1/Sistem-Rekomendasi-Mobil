# img_map.py — merge cars_map.suggest.json -> public/cars_map.json
...
try:
    from images import _index_fs_lower as _fs_index_lower  # {stem_lower: fileActual}
except Exception:
...
def _canon(s: str) -> str:
    """Samakan format supaya 'A7 AT' ~ 'A7AT'; '4 x 4' ~ '4x4'; 'CR-V' ~ 'CRV'; 'e:HEV' ~ 'ehev'."""
    s = str(s)
    s = s.replace("–", "-").replace("—", "-")
...
def _resolve_actual_filename(
    ...
    if verbose:
        print(f"[MISS-V] '{suggest_file}' -> stem='{stem}' canon='{c}' (tidak ada kandidat)")
    return ""

# ------------------ main ------------------
def main():
    ap = argparse.ArgumentParser(description="Apply cars_map.suggest.json ke public/cars_map.json (validasi & normalisasi).")
    ap.add_argument("--suggest", default=os.path.abspath("./reports/cars_map.suggest.json"),
                    help="Path JSON saran map (default: ./reports/cars_map.suggest.json)")
    ap.add_argument("--out", default=os.path.abspath("./public/cars_map.json"),
                    help="Path output cars_map.json (default: ./public/cars_map.json)")
    ...
    add_ok = add_skip_exist = add_missing = 0
    ...
    for key, file_suggest in suggest_map.items():
        ...
        actual = _resolve_actual_filename(
            file_suggest, files_by_lower, stems_by_lower, canon_to_actual, verbose=args.verbose
        )
        if actual:
