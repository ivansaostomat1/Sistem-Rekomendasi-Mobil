...
CANON_NEED = {
    # long trip
    "perjalanan_jauh": "perjalanan_jauh", "long trip": "perjalanan_jauh",
    "long_trip": "perjalanan_jauh", "longtrip": "perjalanan_jauh", "trip jauh": "perjalanan_jauh",
    # short trip / city
    "perkotaan": "perkotaan", "short trip": "perkotaan", "short_trip": "perkotaan",
    "shorttrip": "perkotaan", "city": "perkotaan", "urban": "perkotaan",
    # fun to drive
    "fun": "fun", "fun to drive": "fun", "fun_to_drive": "fun", "fun2drive": "fun", "sporty": "fun",
    # offroad
    "offroad": "offroad", "off road": "offroad", "off_road": "offroad",
    # niaga
    "niaga": "niaga", "usaha": "niaga", "commercial": "niaga",
    # keluarga
    "keluarga": "keluarga", "family": "keluarga",
}
...
def meta():
    ...
    # Kembalikan fuels sebagai list label sederhana untuk UI
    def _fuel_to_code(v: str) -> str:
        s = str(v or "").strip().lower()
        if not s or s in {"na", "n/a", "-"}: return "o"
        if "phev" in s or "plug-in" in s or "plugin" in s or "plug in" in s: return "p"
        if "hybrid" in s or "hev" in s: return "h"
        if "bev" in s or "battery" in s or "electric" in s or s == "ev": return "e"
        if s == "d" or "diesel" in s or "dsl" in s or "solar" in s: return "d"
        if s == "g" or "bensin" in s or "gasoline" in s or "petrol" in s: return "g"
        return "o"

    FUEL_LABEL_SIMPLE = {"g": "Bensin", "d": "Diesel", "h": "Hybrid", "p": "PHEV", "e": "BEV"}

    if "fuel" in specs.columns:
        codes = [_fuel_to_code(x) for x in specs["fuel"].dropna().astype(str).tolist()]
        codes = [c for c in codes if c in FUEL_LABEL_SIMPLE]  # buang "o"
        order = ["Bensin", "Diesel", "Hybrid", "PHEV", "BEV"]
        fuels_strings = sorted({FUEL_LABEL_SIMPLE[c] for c in codes},
...
def recommendations(req: RecommendRequest):
    ...
    try:
        ...
        brand_keys = specs["brand"].astype(str).str.strip().str.upper().unique().tolist()
        retail_share = pd.DataFrame({
            "brand_key": brand_keys,
            "brand_share_ratio": [0.0] * len(brand_keys)
        })
    ...
    if fuels_in:
        filters["fuels"] = [c for c in {fuel_to_code(v) for v in fuels_in} if c in {"g", "d", "h", "p", "e"}]
    ...
    if not isinstance(cand, pd.DataFrame):
        raise HTTPException(status_code=500, detail="rank_candidates mengembalikan nilai tak terduga")
    ...
    # Siapkan kolom tampil & format
    display = [
        "rank","points",
        "brand","model","price",
        "fit_score","fuel","fuel_code",
        "trans","seats","cc_kwh",
        "alasan","image_url","image"
    ]
    for col in display:
    ...
    items = df_to_items(cand[display])
